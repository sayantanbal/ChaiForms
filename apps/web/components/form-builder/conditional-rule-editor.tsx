...
function RuleGroupRenderer({
  ...
  return (
    <div
      className={`flex flex-col gap-3 rounded-md ${!isRoot ? "border border-border p-3 bg-muted/20" : ""}`}
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={addGroup}
            className="text-xs text-secondary-foreground hover:text-secondary-foreground/80 flex items-center gap-1"
          >
        ...
        <div className="space-y-3 pl-4 border-l-2 border-muted">
          {group.rules.map((node: any, index: number) => {
            ...
            return (
              <div
                key={node.id || index}
                className="flex gap-2 items-center bg-muted/50 p-2 rounded border border-border relative group/rule"
              >
                <select
                  value={node.field}
                ...
                {needsValue && (
                  <input
                    ...
                    value={
                      isArrayOp && Array.isArray(node.value)
                        ? node.value.join(", ")
                        : node.value || ""
                    }
                    onChange={(e) => {
